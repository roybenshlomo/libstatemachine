...
{
    return (statemachine_entry_t *)(statemachine->states + (state*event+event));
}
...
statemachine_t * create_statemachine(unsigned int num_states, unsigned int num_events, unsigned int initial_state)
{
    ...
    if (statemachine) {
        ...
        statemachine->states = (statemachine_entry_t *)malloc(sizeof(statemachine_entry_t) * num_states * num_events);
