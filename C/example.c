...
typedef enum states_t { IDLE, RINGING, ACTIVE_CALL, NUM_OF_STATES } states_t;
...
int main(int argc, char *argv[])
{
	...
	statemachine_t *statemachine = create_statemachine(
		NUM_OF_STATES, NUM_OF_EVENTS, IDLE, generic_error);

	// populate the different state machine state,event definitions
	add_event_handler(statemachine, IDLE, HOOK_PICKUP, call_tone,
			  ACTIVE_CALL);
	add_event_handler(statemachine, IDLE, INCOMING_CALL, start_ringing,
			  RINGING);
	add_event_handler(statemachine, RINGING, HOOK_PICKUP, initiate_call,
			  ACTIVE_CALL);
	add_event_handler(statemachine, RINGING, INCOMING_CALL, no_action,
			  RINGING);
	add_event_handler(statemachine, ACTIVE_CALL, HOOK_DOWN, terminate_call,
			  IDLE);
